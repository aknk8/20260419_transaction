...
describe('GET /api/orders', () => {
  it('should return 200 with order list when authenticated', async () => {
    ...
    expect(res.json().data).toEqual([sampleOrder]);
  });
  ...
  it('should return paginated response with data and meta fields', async () => {
    ...
    // Assert
    const body = res.json();
    expect(body.data).toEqual([sampleOrder]);
  ...
  it('should return empty data when page exceeds total pages', async () => {
    ...
    // Act
    const res = await app.inject({ method: 'GET', url: '/api/orders?page=2', cookies: { token: makeToken(app) } });
  ...
  it('should respect limit query parameter', async () => {
    ...
    // Act
    const res = await app.inject({ method: 'GET', url: '/api/orders?limit=1', cookies: { token: makeToken(app) } });
