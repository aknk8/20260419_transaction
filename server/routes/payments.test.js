...
describe('GET /api/payments', () => {
  it('should return 200 with payment list when authenticated', async () => {
    ...
    // Assert
    expect(res.statusCode).toBe(200);
    expect(res.json().data).toEqual([samplePayment]);
  });
  ...
  it('should return paginated response with data and meta fields', async () => {
    ...
    // Assert
    const body = res.json();
    expect(body.data).toEqual([samplePayment]);
  ...
  it('should return empty data when page exceeds total pages', async () => {
    ...
    // Act
    const res = await app.inject({ method: 'GET', url: '/api/payments?page=2', cookies: { token: makeToken(app) } });
  ...
  it('should respect limit query parameter', async () => {
    ...
    // Act
    const res = await app.inject({ method: 'GET', url: '/api/payments?limit=1', cookies: { token: makeToken(app) } });
